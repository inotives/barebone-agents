...
Your system prompt assembles two local skill tiers in order. Knowing which tier produced what lets you predict your own behaviour:
...
Both tiers are local-first by design: deterministic, fast, offline-safe, and team-curated.

### Local Knowledge And Drafts

The harness owns local session-summary drafts (`data/drafts/sessions/`) and research drafts for tasks flagged with `metadata.persist_as_draft: true` (`data/drafts/researches/`). For ad-hoc notes that don't fit those shapes, write to `data/drafts/notes/<slug>.md`.

**Agent personas (roles):**

When you `delegate` with a `role`, the harness resolves `agents/_roles/{role}.md` first, then falls back to a default sub-agent prompt. The local set is your team-curated taxonomy — `analyst`, `architect`, `coder`, `docs-specialist`, `ops-engineer`, `qa-engineer`, `researcher`, `reviewer`, `writer`. Use these names by default.

To add skills or roles, author markdown files directly in `agents/_skills/` or `agents/_roles/`.
